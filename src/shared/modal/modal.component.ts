...
@Component({
  ...
  styles: [`
    ...
    .dialog-surface { background: #ececec; color: var(--text); border-radius: 22px; width: 100%; max-width: 560px; max-height: calc(100vh - 32px); display: flex; flex-direction: column; box-shadow: 0 28px 80px rgba(0,0,0,.35); animation: in .15s ease-out; overflow: hidden; }
    @keyframes in { from { transform: scale(.96); opacity: 0 } to { transform: scale(1); opacity: 1 } }
    header { display: flex; justify-content: space-between; align-items: center; padding: 18px 22px; background: linear-gradient(180deg, #f7f7f7 0%, #e9e9e9 100%); border-bottom: 1px solid rgba(0,0,0,.06); flex-shrink: 0; }
    ...
    .body { padding: 22px; overflow-y: auto; flex: 1; }
  `],
})
